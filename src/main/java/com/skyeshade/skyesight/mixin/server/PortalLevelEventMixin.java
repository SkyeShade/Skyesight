...
import com.skyeshade.skyesight.network.SkyesightLevelEventPayload;
...
public abstract class PortalLevelEventMixin {
    ...
    private void skyesight$effect(Player excluded, int id, BlockPos pos, int data, CallbackInfo ci) {
        ...
        for (var watched : SkyesightServerViewTracker.viewsWatching(level.dimension(), new ChunkPos(pos))) {
            ...
            var context = player == null ? null : PortalInteractionContext.forEntity(player);
            boolean vanillaDelivered = player != null && (player == excluded
                    ? context == null // Ordinary physical mining predicted its own break event.
                    : player.level() == level && player.position().distanceToSqr(Vec3.atCenterOf(pos)) < 64 * 64);
            if (player != null) PacketDistributor.sendToPlayer(player,
                    new SkyesightLevelEventPayload(watched.watch().viewId(), watched.watch().generation(), level.dimension(), pos, id, data, sequence, vanillaDelivered));
        }
    }
}
