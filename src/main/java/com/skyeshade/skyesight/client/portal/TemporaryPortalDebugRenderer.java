...
public final class TemporaryPortalDebugRenderer {
    ...
    private TemporaryPortalDebugRenderer() {}

    public static void renderPortalDebug(
            Camera mainCamera,
    ...
    private static void drawPortalAxes(
        ...
        Vec3 right = TemporaryPortalMath.right(portal);
        Vec3 up = TemporaryPortalMath.up(portal);
        Vec3 normal = TemporaryPortalMath.normal(portal);
    ...
    private static PortalCorners corners(TemporaryPortalFrame portal) {
        Vec3 center = portal.position();

        Vec3 right = TemporaryPortalMath.right(portal).scale(-1.0D);
        Vec3 up = TemporaryPortalMath.up(portal);
