...
public final class PortalRegionTraversal {
    private record Sample(ResourceKey<Level> dimension,Vec3 position) {}
    ...
    public static void movementAccepted(ServerPlayer player) {
        var id=player.getUUID();var current=player.position();
        var previous=PREVIOUS.put(id,new Sample(player.level().dimension(),current));
        ...
        for(var direction:PortalRegionManager.directions()) {
            ...
            var source=direction.source();var a=d.source().local(previous.position);var b=d.source().local(current);
            ...
            if(speed*b.z<0 && Math.abs(b.z)<Math.max(8,Math.min(d.activationDistance(),Math.abs(speed)*20))) {
                var predicted=current.add(velocity.scale(Math.min(20,Math.abs(b.z/speed))));
            ...
            var state=states.computeIfAbsent(key,k->{var s=new PortalCrossingState();s.update(source,previous.position,d::containsWorld);return s;});
            int side=state.stableSide();
            var hit=state.update(source,current,d::containsWorld);
            if(hit!=null && d.sidedness().allows(side==0?a.z:side)
                    && (closestHit==null || previous.position.distanceToSqr(hit)<previous.position.distanceToSqr(closestHit))) {
                closest=direction;closestHit=hit;
            }
        }
        if(closest==null) return;
        var source=closest.source();var target=closest.target();
        var pose=PortalTraversalMath.transform(source,target,current,velocity,player.getYRot(),player.getXRot());
        ...
        PREVIOUS.put(id,new Sample(target.dimension(),pose.position())); GUARDS.put(id,target);
